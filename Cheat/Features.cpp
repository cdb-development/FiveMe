...
void CFramework::MiscAll() // This is where we put all the misc features that require constant updating
{
    // GodMode
...
void CFramework::UpdateList() // This is where we update the entity list every 500ms
{
    while (g.Run)
    {
...
Vector3 GetPrediction(CPed& target, CPed& local) // This is where we calculate the prediction. It's really bad right now, so we should come up with a better idea.
{
    Vector3 vOut{};
    ...
    if (BulletSpeed == 0.f)
        ...
        BulletSpeed = 1800.f; // tentative

    // This is just a placeholder. We should come up with a better idea.
