...
bool CPed::Update() // Update PedData
{
...
bool CPed::IsDead() // Check if ped is dead
{
    return m_fHealth <= 0.f || Vec3_Empty(m_pVecLocation);
}

bool CPed::IsPlayer() // Check if ped is player
{
    return PlayerInfo != NULL;
}

bool CPed::InVehicle() // Check if ped is in vehicle
{
    return m.Read<uint8_t>(address + sdk.m_bPedTask) & (uint8_t)ePedTask::TASK_DRIVING;
}

bool CPed::IsGod() // Check if ped is in godmode
{
    return m.Read<bool>(address + sdk.m_bGodMode) == true;
}

struct Bone { // Bone struct
    Vector3 pos{};
    int junk0{};
};

struct AllBone { // All bone struct
    Bone bone[9]{};
};

std::vector<Vector3> CPed::GetBoneList() // Get list of ped bones
{
...
Vector3 CPed::GetVelocity() // Get ped velocity
{
