...
void CFramework::RenderInfo()
{
    ...
    float posX = (screenWidth - textSize.x) / 2.f;
    float posY = 8.f;

    // Box Hintergrund
    ImVec2 boxSize = ImVec2(textSize.x + 20.f, textSize.y + 8.f);
    ImGui::GetBackgroundDrawList()->AddRectFilled(
        ImVec2(posX - 10.f, posY - 4.f),
        ImVec2(posX + textSize.x + 10.f, posY + textSize.y + 4.f),
        ImColor(0.f, 0.f, 0.f, 0.5f),
        6.f  // Abrundung
    );

    // Text mit Schatten
    StringEx(ImVec2(posX + 1.f, posY + 1.f), ImColor(0.f, 0.f, 0.f, 0.7f), ImGui::GetFontSize(), fpsText);
    StringEx(ImVec2(posX, posY), ImColor(1.f, 1.f, 1.f, 1.f), ImGui::GetFontSize(), fpsText);
